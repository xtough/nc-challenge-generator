...
The system SHALL allow configuration of how many items to suggest per category. Artist count is always 1 per challenge.

#### Scenario: Default item count
- **WHEN** no configuration is specified
- **THEN** system uses 5 items per category (except mandatory keyword) and 1 artist

#### Scenario: Custom item count
- **WHEN** user runs `nightcafe-gen --items 10`
- **THEN** system generates challenge with 10 items per category and 1 artist

#### Scenario: Override specific category
- **WHEN** user runs `nightcafe-gen --subjects 3 --artists 1`
- **THEN** subjects category has 3 suggestions, artist category has 1 (always), and other categories have default 5

#### Scenario: Artist count always one
- **WHEN** user attempts to override artist count with `--artists 5`
